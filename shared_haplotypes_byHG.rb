  ...
  unique_haplotypes.each_pair do |population, haplogroups|
    haplogroups.each_pair do |haplogroup, haplotypes|
      ...
      other_populations.each do |other_population|
        unique_haplotypes[other_population][haplogroup].each do |haplotype|
          shared_haplotypes[[population, other_population]][haplogroup] << haplotype if haplotypes.include?(haplotype)
        end
      end
    end
  end
...
def write_shared_haplotypes(shared_haplotypes, filename)
  CSV.open(filename, 'w') do |csv|
    csv << ['Pop1', 'Pop2', 'HG', 'Shared haplotype']
    shared_haplotypes.each_pair do |(pop1, pop2), haplogroups|
      haplogroups.each_pair do |haplogroup, haplotypes|
        next if haplotypes.empty?
        haplotypes.each { |haplotype| csv << [pop1, pop2, haplogroup, haplotype] }
      end
    end
  end
end
...
def write_summary(summary, filename)
  CSV.open(filename, 'w') do |csv|
    csv << ['Pop1', 'Pop2', 'HG', 'Number of Shared haplotypes']
