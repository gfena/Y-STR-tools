...
infile=args[1] # input
out_file_prefix = args[2] # output file prefix
...
output_file_name = paste(out_file_prefix, "ID_byHaplotype.csv")
output_file_name = basename(output_file_name)
write.table(individuals_per_haplotype, file = output_file_name, sep = "\t", row.names = FALSE, quote = FALSE)
...
output_file_name = paste(out_file_prefix, "logscaledweights.csv")
output_file_name = basename(output_file_name)
write.csv(weights_df, file = output_file_name, row.names = FALSE, quote = FALSE)
